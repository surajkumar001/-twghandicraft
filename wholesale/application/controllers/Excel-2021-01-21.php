...
class Excel extends CI_Controller {
	...
	function __construct() {
         ...
         $this->load->model('Adminmodel');
         	...
         	$url = 'http://' . $_SERVER['HTTP_HOST'] . "/wholesaledev/phpexcel_gd/";

        	$path = $_SERVER['DOCUMENT_ROOT'] . '/wholesaledev/phpexcel_gd/';
...
function xlscreation_direct($data) {
	...
	foreach ($result as  $value){
		...
		$img = 'http://adsfly.in/wholesale/assets/product/'.$product_detail->main_image1;
		...
		$propertydetails[] =array('SNo' => $i++,'Image' => str_replace('http://adsfly.in/wholesale/','.',$img),'ProductSKU' => $value['product_id'],'DemandedQty' => $value['customer_quantity'],'DispatchedQty' => $value['quantity'],'DiscWPbeforetax' => $value['price_after_discount'],'NetPrice' => $real_price,'Amount' => $sub_price,'GST' => $value['gst']."%",'GSTAmount' => $gst_val);
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A5',$htmlHelper->toRichTextObject('<b>Business Name : </b>'.$customeradd->customer_business));
	...
	$objPHPExcel->getActiveSheet()->setCellValue('F3',$htmlHelper->toRichTextObject('<b>Address : </b>'.$result[0]['cutomer_address']));
	$objPHPExcel->getActiveSheet()->mergeCells('F4:J4');
	$objPHPExcel->getActiveSheet()->setCellValue('F4',$htmlHelper->toRichTextObject('<b>PinCode : </b>'.$result[0]['pincode']));
	$pin =  $result[0]['pincode'];
	$pincode = $this->db->get_where('pincode' , array('name' => $pin  ))->row() ;
	$objPHPExcel->getActiveSheet()->mergeCells('F5:J5');
	$objPHPExcel->getActiveSheet()->setCellValue('F5',$htmlHelper->toRichTextObject('<b>City : </b>'.$pincode->area));
...
$table_bg_arr=array(
	...
	if(!empty($totalgst_5)){
		...
		if($state != 'UTTAR PRADESH'){
			...
			$gst_5 = $totalgst_5;
			$objPHPExcel->getActiveSheet()->setCellValue('H'.($cell + 8),$gst_5);
		}
		$cell++;
	}
	$objPHPExcel->getActiveSheet()->setCellValue('B'.($cell + 8),'GST @12%');
	$objPHPExcel->getActiveSheet()->setCellValue('B'.($cell + 9),'GST @18%');
	if(!empty($totalgst_28)){
		$objPHPExcel->getActiveSheet()->setCellValue('B'.($cell + 10),'GST @28%');
	    //$objPHPExcel->getActiveSheet()->getStyle('D'.($cell + 10))->applyFromArray($yellow_color);
		if($state != 'UTTAR PRADESH'){
			$objPHPExcel->getActiveSheet()->setCellValue('D'.($cell + 10),'IGST : '.$totalgst_28);
		} else {
	    	//$objPHPExcel->getActiveSheet()->getStyle('E'.($cell + 10))->applyFromArray($yellow_color);
			$objPHPExcel->getActiveSheet()->setCellValue('D'.($cell + 10),'CGST : '.round($totalgst_28/2,2));
			$objPHPExcel->getActiveSheet()->setCellValue('E'.($cell + 10),'SGST : '.round($totalgst_28/2 ,2));
			$gst_28 = $totalgst_28;
			$objPHPExcel->getActiveSheet()->setCellValue('H'.($cell + 10),$gst_28);
		}
		$cell++;
	}
	$objPHPExcel->getActiveSheet()->setCellValue('B'.($cell + 10),'Total Invoice Value');
	$objPHPExcel->getActiveSheet()->setCellValue('B'.($cell + 11),'Advance');
	$objPHPExcel->getActiveSheet()->setCellValue('B'.($cell + 12),'Used Credit');
	$objPHPExcel->getActiveSheet()->setCellValue('B'.($cell + 13),'Balance 2nd Payment Received');
	$objPHPExcel->getActiveSheet()->setCellValue('B'.($cell + 14),'Balance 3rd Payment Received');
	$objPHPExcel->getActiveSheet()->setCellValue('B'.($cell + 15),'Balance Amount');
	$objPHPExcel->getActiveSheet()->getRowDimension($cell + 15)->setRowHeight(30); 
	...
	//$objPHPExcel->getActiveSheet()->getStyle('D'.($cell + 8))->applyFromArray($yellow_color);
	if($state != 'UTTAR PRADESH'){
		$objPHPExcel->getActiveSheet()->setCellValue('D'.($cell + 8),'IGST : '.$gst_12);
	} else {
	    //$objPHPExcel->getActiveSheet()->getStyle('E'.($cell + 8))->applyFromArray($yellow_color);
		$objPHPExcel->getActiveSheet()->setCellValue('D'.($cell + 8),'CGST : '.round($gst_12/2,2));
		$objPHPExcel->getActiveSheet()->setCellValue('E'.($cell + 8),'SGST : '.round($gst_12/2,2));
	}
	...
	if($state != 'UTTAR PRADESH'){
		$objPHPExcel->getActiveSheet()->setCellValue('D'.($cell + 9),'IGST : '.$gst_18);
	} else {
	    //$objPHPExcel->getActiveSheet()->getStyle('E'.($cell + 9))->applyFromArray($yellow_color);
		$objPHPExcel->getActiveSheet()->setCellValue('D'.($cell + 9),'CGST : '.round($gst_18/2,2));
		$objPHPExcel->getActiveSheet()->setCellValue('E'.($cell + 9),'SGST : '.round($gst_18/2,2));
	}
	$payment_mode= $result[0]['payment_mode'];
	$objPHPExcel->getActiveSheet()->setCellValue('D'.($cell + 11),'Payment Mode : '.$payment_mode);
	...
	$objPHPExcel->getActiveSheet()->setCellValue('E'.($cell + 11),$transaction_id);
	if($result[0]['confirm_payment'] =='done')$confirm_payment='Verified';
	else $confirm_payment='Not Verified';
	$objPHPExcel->getActiveSheet()->setCellValue('F'.($cell + 11),$confirm_payment);
	...
	$objPHPExcel->getActiveSheet()->setCellValue('D'.($cell + 14),'Payment Mode : '.$payment_mode);
    $transaction_id='';
	if($payment_mode == 'Online' ) {
		$transaction_id=$result[0]['online_transaction_id']; 
	}else{
	...
	$objPHPExcel->getActiveSheet()->setCellValue('E'.($cell + 14),$transaction_id);
	if($pend_3_row->confirm_payment == 'done' )$confirm_payment='Verified';
	else $confirm_payment='Not Verified';
	$objPHPExcel->getActiveSheet()->setCellValue('F'.($cell + 14),$confirm_payment);
...
// 	$order_processing=$result[0]['order_processing'];
	...
	$objPHPExcel->getActiveSheet()->setCellValue('H'.($cell + 8),$gst_12);
	$objPHPExcel->getActiveSheet()->setCellValue('H'.($cell + 9),$gst_18);
	$invoice = $net_amount +  $gst_5 + $gst_12 +  $gst_18+ $gst_28;
	//$objPHPExcel->getActiveSheet()->getStyle('H'.($cell + 10))->applyFromArray($yellow_color);
	$objPHPExcel->getActiveSheet()->setCellValue('H'.($cell + 10),$invoice);
	$advance = $data['order']->advance_payment;
	$objPHPExcel->getActiveSheet()->setCellValue('H'.($cell + 11),$advance);
	$pend_amount = $pend_row->pend_amount;
	$objPHPExcel->getActiveSheet()->setCellValue('H'.($cell + 13),$pend_amount);
	$pend_3_amount = $pend_3_row->pend_amount;
	$objPHPExcel->getActiveSheet()->setCellValue('H'.($cell + 14),$pend_3_amount);
	$final = $invoice - $advance -$pend_amount -$pend_3_amount ;
	$objPHPExcel->getActiveSheet()->setCellValue('H'.($cell + 15),round($final ,2 ));
